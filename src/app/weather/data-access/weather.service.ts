...
export class WeatherService {
  ...
  savedWeather$: BehaviorSubject<WeatherData[]> = new BehaviorSubject<
    WeatherData[]
  ...
  searchWeatherByCity(cityName = ''): Observable<WeatherData> {
    ...
    return this.http
      .get<WeatherDataResponse>(this.WEATHER_URL, {
  ...
  public saveWeatherDataToLocalStorage(data: WeatherData) {
    let savedData = this.localStorage.getItem(this.WEATHER_TOKEN);
    let savedDataParsed = savedData
      ? (JSON.parse(savedData) as WeatherData[])
      : [];
